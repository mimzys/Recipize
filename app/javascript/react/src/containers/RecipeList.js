...
const RecipeList = props => {
  ...
  let ingredientList = props.ingredients.map((ingr, index) => {
    return <li key={index}>{ingr.text}</li>;
  });

  return (
    <div className="recipelist">
      <div
        className="grid-y grid-container-fluid"
        onClick={() =>
          props.dispatch({ type: "SET_RECIPE_SHOW", id: props.index })
        }
      >
        <Link
          to={{
            pathname: `${window.location.pathname}/${props.index}${
